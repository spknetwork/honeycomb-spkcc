...
const calculateCollateralHealth = (provider, stats) => {
  ...
  const blocksDelenquint = stats.pendingblock -  provider.lastActivity > 100 ? 
...
/**
 * Distribute tokens to collateral providers based on health scores
 * @param {object} dex - DEX object with pool
 * @param {number} tokensToDistribute - Amount of tokens to distribute
 * @param {object} collateralProviders - Map of provider addresses to provider data
 * @returns {object} Distribution results
 */
const distributeToCollateralProviders = (dex, tokensToDistribute, collateralProviders) => {
  const distribution = {};
  let totalHealthScore = 0;
  
  // Calculate total health score
  for (const [address, provider] of Object.entries(collateralProviders)) {
    const healthScore = calculateCollateralHealth(provider);
    if (healthScore > 0) {
      distribution[address] = { healthScore, tokens: 0 };
      totalHealthScore += healthScore;
    }
  }
  
  // Distribute tokens proportionally to health scores
  if (totalHealthScore > 0) {
    for (const [address, data] of Object.entries(distribution)) {
      data.tokens = Math.floor((data.healthScore / totalHealthScore) * tokensToDistribute);
    }
    
    // Handle rounding remainder
    const distributed = Object.values(distribution).reduce((sum, d) => sum + d.tokens, 0);
    const remainder = tokensToDistribute - distributed;
    if (remainder > 0) {
      // Give remainder to highest health score provider
      const topProvider = Object.entries(distribution)
        .sort(([, a], [, b]) => b.healthScore - a.healthScore)[0];
      if (topProvider) {
        distribution[topProvider[0]].tokens += remainder;
      }
    }
  }
  
  // Update pool tracking
  if (!dex.pool.distributions) {
    dex.pool.distributions = [];
  }
  dex.pool.distributions.push({
    block: Date.now(), // Should use block number in production
...
const addReserveToBondingCurve = (dex, reserveAmount, pair, stats, collateralProviders) => {
  ...
  // Distribute portion of minted tokens to collateral providers
  const providerShare = Math.floor(tokensToMint * 0.1); // 10% to providers
  const poolShare = tokensToMint - providerShare;
  
  const distribution = distributeToCollateralProviders(dex, providerShare, collateralProviders);
