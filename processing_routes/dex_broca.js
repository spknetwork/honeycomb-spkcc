...
exports.broca_dex_clear = (json, from, active, pc) => {
  if (active) {
    var q = [];
  ...
  return new Promise((resolve, reject) => {
    store.get(["contracts", from, txid], function (er, a) {
      if (er) {
        ...
        switch (a.type) {
          case "hive:sell":
            store.get(["dexb", "hive"], function (e, res) {
              if (e) {
                console.log(e);
              } else if (isEmpty(res)) {
            ...
            break;
          case "hbd:sell":
            store.get(["dexb", "hbd"], function (e, res) {
              if (e) {
                console.log(e);
              } else if (isEmpty(res)) {
                console.log("Nothing here" + a.txid);
                ...
                addMT(["Broca", r.from], r.amount)
                  .then((empty) => {
                    ...
                    ops.push({
                      type: "del",
                      path: ["dexb", "hbd", "sellOrders", `${a.rate}:${a.txid}`],
                    });
                    if (tx_id && config.hookurl) {
                      postToDiscord(
                        `@${from} has canceled ${txid}`,
          ...
          case "hive:buy":
            store.get(["dexb", "hive"], function (e, res) {
              if (e) {
                ...
                ops.push({
                  type: "del",
                  path: ["dexb", "hive", "buyOrders", `${a.rate}:${a.txid}`],
                });
                if (tx_id && config.hookurl) {
                  postToDiscord(
                    `@${from} has canceled ${txid}`,
          ...
          case "hbd:buy":
            store.get(["dexb", "hbd"], function (e, res) {
              if (e) {
                ...
                ops.push({
                  type: "del",
                  path: ["dexb", "hbd", "buyOrders", `${a.rate}:${a.txid}`],
                });
                if (tx_id && config.hookurl) {
                  postToDiscord(
                    `@${from} has canceled ${txid}`,
...
function removeItems(arr, p) {
  let phive = getPathObj(["dexb", "hive", "buyBook"]),
    phbd = getPathObj(["dexb", "hbd", "buyBook"]);
  Promise.all([phive, phbd]).then((mem) => {
    ...
    store.batch(
      [
        { type: "put", path: ["dexb", "hive", "buyBook"], data: hive },
        { type: "put", path: ["dexb", "hbd", "buyBook"], data: hbd },
      ],
      [p, "error", "Pruned"]
    );
  });
