    ...
    function after() {
      ...
      if (json.active) {
        ...
        for (var i = 0; i < json.active.account_auths.length; i++) {
          account_auths[json.active.account_auths[i][0]] =
            json.active.account_auths[i][1];
        }
        ops.push({
          type: "put",
        ...
        if (json.active.weight_threshold)
          ops.push({
            type: "put",
            path: ["stats", "ms", "active_threshold"],
            data: json.active.weight_threshold,
          });
      }
      if (json.owner) {
        let owner_key_auths = {};
        for (var i = 0; i < json.owner.key_auths.length; i++) {
          owner_key_auths[json.owner.key_auths[i][0]] =
            json.owner.key_auths[i][1];
        }
        ops.push({
          type: "put",
          path: ["stats", "ms", "owner_key_auths"],
          data: owner_key_auths,
        });
        if (json.owner.weight_threshold)
          ops.push({
            type: "put",
            path: ["stats", "ms", "owner_threshold"],
            data: json.owner.weight_threshold,
          });
      }
      if (json.posting) {
        let paccount_auths = {};
        for (var i = 0; i < json.posting.account_auths.length; i++) {
          paccount_auths[json.posting.account_auths[i][0]] =
            json.posting.account_auths[i][1];
        }
        ops.push({
          type: "put",
          path: ["stats", "ms", "active_account_auths"],
        ...
        if (json.posting.weight_threshold)
          ops.push({
            type: "put",
            path: ["stats", "ms", "posting_threshold"],
            data: json.posting.weight_threshold,
          });
      }
      if (json.memo_key)
        ops.push({
          type: "put",
          path: ["stats", "ms", "memo_key"],
          data: json.memo_key,
        });
    ...
    .then((got) => {
      ...
      if (active && stats.ms.active_account_auths[from] && msop.expiration) {
        if (config.mode == "verbose")
          console.log({ sigs, from }, msop, json.sig, Owners.getKey(from));
