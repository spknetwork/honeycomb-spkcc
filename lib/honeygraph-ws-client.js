...
class HoneygraphWSClient extends EventEmitter {
  ...
  connect() {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.on('open', () => {
          console.log(`[HoneygraphWS] WebSocket opened to ${wsUrl}`);
          console.log('[HoneygraphWS] Connected - Waiting for server messages...');
          this.isConnected = true;
          this.reconnectAttempts = 0;
          this.currentReconnectInterval = this.config.reconnectInterval;
          
          // Don't send anything yet - wait for auth_required or welcome message
          this.startHeartbeat();
          this.emit('connected');
          resolve();
        });
  ...
  handleMessage(message) {
    console.log(`[HoneygraphWS] Received message type: ${message.type}`);
    
    switch (message.type) {
      case 'auth_required':
        console.log('[HoneygraphWS] Authentication required by server');
        console.log('[HoneygraphWS] Challenge:', JSON.stringify(message.challenge));
        this.emit('auth_required', message);
        break;
        
      case 'auth_success':
        console.log('[HoneygraphWS] Authentication successful');
        this.emit('auth_success', message);
        break;
        
      case 'auth_failed':
        console.log('[HoneygraphWS] Authentication failed');
        console.log('[HoneygraphWS] Failure reason:', message.reason || 'Unknown');
        if (message.details) {
          console.log('[HoneygraphWS] Failure details:', message.details);
        }
        this.emit('auth_failed', message);
        break;
        
      case 'welcome':
        console.log('[HoneygraphWS] Received welcome message from server');
        this.emit('welcome', message);
        break;
        
      case 'request_missing':
        console.log(`[HoneygraphWS] Server requesting missing operations from index ${message.from} to ${message.to}`);
      ...
      case 'sync_status':
        console.log(`[HoneygraphWS] Server sync status - lastIndex: ${message.lastIndex}`);
