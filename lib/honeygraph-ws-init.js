...
import HoneygraphWSClient from './honeygraph-ws-client.js';
...
class HoneygraphWSIntegration {
  ...
  async initialize() {
    ...
    if (this.client) {
      console.log('[HoneygraphWS] Already initialized, skipping');
      return;
    }
  ...
  trackOperation(operation) {
    ...
    if (typeof operation.index === 'undefined' || typeof operation.blockNum === 'undefined') {
  ...
  cleanupOldBlocks(currentBlockNum) {
    const blocksToKeep = 300; // Keep last 300 blocks (3 checkpoint periods) for catch-up
