...
export var TXID = {
  ...
  reset: function () {
    TXID.streaming = false, TXID.blocknumber = 0, saveNumber = 0, status = {
      cleaner: [],
    }
  },
}
...
import IPFS from "ipfs-http-client-lite"; //ipfs-http-client doesn't work
import hive from '@hiveio/dhive';
export var client = new hive.Client(config.clients);
import HIVEClient from '@hiveio/hive-js';
export const hiveClient = HIVEClient
export var ipfs = IPFS(
...
export var plasma = {
  ...
  jwt;
var NodeOps = [];
export const GetNodeOps = function () { return NodeOps }
export const newOps = function (array) { NodeOps = array }
export const unshiftOp = function (op) { NodeOps.unshift(op) }
export const pushOp = function (op) { NodeOps.push(op) }
...
export var escrow = false;
export var processor
export var waitfor
import cors from 'cors';
import broadcastClient from '@hiveio/hive-js';
...
Promise.all([config.startURL, config.clientURL]).then(urls => {
  ...
  const statestart = config.stateStart
  ...
  if (config.features.pob) {
    ...
    api.get('/dapps/@:author', API.wrapGetAuthorPosts);
    api.get('/dapps/@:author/:permlink', API.wrapGetPost);
    api.get('/new', API.wrapGetNewPosts);
    api.get('/trending', API.wrapGetTrendingPosts);
    api.get('/promoted', API.wrapGetPromotedPosts);
  ...
  function startApp(state) {
    ...
    if (config.features.pob) {
      ...
      //processor.on('cjv', HR.cjv); // adjust post data 
    ...
    var chronOps = {
      ...
      post_reward: function (b, passed, res, rej, num, prand, ints) {
      ...
      gov_down: function (b, passed, res, rej, num, prand, ints) {
      ...
      denyT: function (b, passed, res, rej, num, prand, ints) {
        enforce(b.agent, b.txid, { id: b.id, acc: b.acc }, num)
        store.batch([{ type: 'del', path: ['chrono', passed.delKey] }], [res, rej, 'info'])
      },
      denyA: function (b, passed, res, rej, num, prand, ints) {
        enforce(b.agent, b.txid, { id: b.id, acc: b.acc }, num)
        store.batch([{ type: 'del', path: ['chrono', passed.delKey] }], [res, rej, 'info'])
      },
      check: function (b, passed, res, rej, num, prand, ints) {
        enforce(b.agent, b.txid, { id: b.id, acc: b.acc }, num)
        store.batch([{ type: 'del', path: ['chrono', passed.delKey] }], [res, rej, 'info'])
      },
      expire: function (b, passed, res, rej, num, prand, ints) {
        release(b.from, b.txid, num)
        store.batch([{ type: 'del', path: ['chrono', passed.delKey] }], [res, rej, 'info'])
      },
      ms_send: function (b, passed, res, rej, num, prand, ints) {
        //recast(b.attempts, b.txid, num)
        store.batch([{ type: 'del', path: ['chrono', passed.delKey] }], [res, rej, 'info'])
      },
      del_pend: function (b, passed, res, rej, num, prand, ints) {
        store.batch([{ type: 'del', path: ['chrono', passed.delKey] }, { type: 'del', path: ['pend', `${b.author}/${b.permlink}`] }], [res, rej, 'info'])
      },
      div: function (b, passed, res, rej, num, prand, ints) {
      ...
      ame: function (b, passed, res, rej, num, prand, ints) {
      ...
      ahe: function (b, passed, res, rej, num, prand, ints) {
      ...
      mint: function (b, passed, res, rej, num, prand, ints) {
              ...
              function ChonOp(delKey, ints, prand, num, bh) {
                return new Promise((res, rej) => {
                  store.getWith(['chrono', chrops[j[i]]], { delKey, ints }, function (e, b, passed) {
                    if(chronOps[b.op])console.log(b, passed, res, rej, num, prand, ints, bh)
                  ...
                  if (plasma.hashBlock < num - 100) {
                    promises.push(new Promise((res, rej) => {
                      function waitforipfs(n) {
                        if (plasma.hashBlock < num - 150) {
                          setTimeout(() => {
                            waitforipfs(n + 1)
                          }, 1000)
                          console.log(`Waiting for IPFS ${n} second${n > 1 ? 's' : ''}`)
                        } else {
                          res('SAT')
                        }
                      }
  ...
  waitfor = function (promises_array) {
    return new Promise((resolve, reject) => {
                  ...
                  if (!e && (second || data[0] > API.RAM.head - 325)) {
                    if (hash) {
                      ...
                      if (cleanState.stats.chain){
                        ...
                        tag: config.tag,
                        jsonTokenName: config.jsonTokenName,
                        leader: config.leader,
                        ben: config.ben,
                        delegation: config.delegation,
                        delegationWeight: config.delegationWeight,
                        msaccount: config.msaccount,
                        msPubMemo: config.msPubMemo,
                        msPriMemo: config.msPriMemo,
                        msmeta: config.msmeta,
                        mainAPI: config.mainAPI,
                        mainRender: config.mainRender,
                        mainFE: config.mainFE,
                        mainIPFS: config.mainIPFS,
                        mainICO: config.mainICO,
      ...
      store.del([], function (e) {
        ...
        store.put([], statestart, function (err) {
          if (err) {
            ...
            store.get([""], function (error, returns) {
              if (!error) {
                console.log(
                  `State Check:  ${returns.stats.hashLastIBlock}\nAccount: ${config.username
                  }\nKey: ${config.active.substr(0, 3)}...`
                );
              }
            });
  ...
  function unwrapOps(arr) {
    return new Promise((resolve, reject) => {
      ...
      if (arr && arr.length && arr[arr.length - 1] !== "W") arr.push("W");
      if (arr && arr.length) write(0);
      else resolve([]);
      function write(int) {
        d = [];
